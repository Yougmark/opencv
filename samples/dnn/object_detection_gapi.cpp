...
int main(int argc, char** argv)
{
    ...
    std::thread processingThread([&](){
        ...
        while (process)
        {
            ...
            if (!frame.empty())
            {
                ...
                if (asyncNumReq)
                {
                    printf("async forward in thread\n");
                ...
                else
                {
                    ...
                    // Create a graph of only one node that executes net.forward
                    printf("sync forward in thread as a graph\n");
                    cv::GMat in;
                    cv::GMat out  = cv::gapi::convolve(in, &net, &outs, &outNames);
                    cv::GComputation ac(in, out);
                    cv::Mat output_frame;
                    ac.apply(frame, output_frame);
    ...
    while (waitKey(1) < 0)
    {
        ...
        std::vector<Mat> outs;
        printf("sync forward in main thread\n");
        net.forward(outs, outNames);
