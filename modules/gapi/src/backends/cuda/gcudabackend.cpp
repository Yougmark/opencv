...
#include "backends/cuda/gcudabackend.hpp"
...
using GCUDAModel = ade::TypedGraph
    < cv::gimpl::Unit
    , cv::gimpl::Protocol
    >;

// FIXME: Same issue with Typed and ConstTyped
using GConstGCUDAModel = ade::ConstTypedGraph
    < cv::gimpl::Unit
    , cv::gimpl::Protocol
    >;
...
namespace
{
    class GCUDABackendImpl final: public cv::gapi::GBackend::Priv
    {
        ...
        {
            GCUDAModel gm(graph);
            auto cuda_impl = cv::util::any_cast<cv::GCUDAKernel>(impl.opaque);
            gm.metadata(op_node).set(cv::gimpl::Unit{cuda_impl});
        }
        ...
        {
            return EPtr{new cv::gimpl::GCUDAExecutable(graph, nodes)};
        }
   };
}

cv::gapi::GBackend cv::gapi::cuda::backend()
{
    static cv::gapi::GBackend this_backend(std::make_shared<GCUDABackendImpl>());
    return this_backend;
}

// GCUDAExecutable implementation //////////////////////////////////////////////
cv::gimpl::GCUDAExecutable::GCUDAExecutable(const ade::Graph &g,
...
cv::GArg cv::gimpl::GCUDAExecutable::packArg(const GArg &arg)
{
    ...
    // Wrap associated CUDA object (either host or an internal one)
...
void cv::gimpl::GCUDAExecutable::run(std::vector<InObj>  &&input_objs,
                                    std::vector<OutObj> &&output_objs)
{
    ...
    for (auto &op_info : m_script)
    {
        ...
        GCUDAKernel k = gcm.metadata(op_info.nh).get<Unit>().k;

        // Initialize kernel's execution context:
        // - Input parameters
        GCUDAContext context;
        context.m_args.reserve(op.args.size());

        using namespace std::placeholders;
        ade::util::transform(op.args,
                          std::back_inserter(context.m_args),
                          std::bind(&GCUDAExecutable::packArg, this, _1));
