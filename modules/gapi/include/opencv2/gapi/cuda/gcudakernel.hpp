...
#ifndef OPENCV_GAPI_GCUDAKERNEL_HPP
#define OPENCV_GAPI_GCUDAKERNEL_HPP
...
namespace cuda
{
    ...
    /**
     * @brief Get a reference to CUDA (OpenCV) backend.
     *
...
// Represents arguments which are passed to a wrapped CUDA function
// FIXME: put into detail?
class GAPI_EXPORTS GCUDAContext
{
public:
...
protected:
    ...
    //to CUDA kernel. (This can be achieved by a two single time conversions in GCUDAExecutable::run,
    //once on enter for input and output arguments, and once before return for output arguments only
    std::unordered_map<std::size_t, GRunArgP> m_results;

    friend class gimpl::GCUDAExecutable;
};

class GAPI_EXPORTS GCUDAKernel
{
public:
    // This function is kernel's execution entry point (does the processing work)
    using F = std::function<void(GCUDAContext &)>;

    GCUDAKernel();
    explicit GCUDAKernel(const F& f);

    void apply(GCUDAContext &ctx);
...
{
    static cv::Mat    get(GCUDAContext &ctx, int idx) { return ctx.inMat(idx); }
};
template<> struct get_in<cv::GMatP>
{
    static cv::Mat    get(GCUDAContext &ctx, int idx) { return get_in<cv::GMat>::get(ctx, idx); }
};
...
{
    static cv::Scalar get(GCUDAContext &ctx, int idx) { return ctx.inVal(idx); }
};
template<typename U> struct get_in<cv::GArray<U> >
{
    static const std::vector<U>& get(GCUDAContext &ctx, int idx) { return ctx.inArg<VectorRef>(idx).rref<U>(); }
};
template<typename U> struct get_in<cv::GOpaque<U> >
{
    static const U& get(GCUDAContext &ctx, int idx) { return ctx.inArg<OpaqueRef>(idx).rref<U>(); }
};
...
template<class T> struct get_in
{
    static T get(GCUDAContext &ctx, int idx) { return ctx.inArg<T>(idx); }
};
...
{
    static tracked_cv_mat get(GCUDAContext &ctx, int idx)
    {
...
{
    static cv::Scalar& get(GCUDAContext &ctx, int idx)
    {
...
{
    static std::vector<U>& get(GCUDAContext &ctx, int idx)
    {
...
{
    static U& get(GCUDAContext &ctx, int idx)
    {
...
struct CUDACallHelper;

// FIXME: probably can be simplified with std::apply or analogue.
template<typename Impl, typename... Ins, typename... Outs>
struct CUDACallHelper<Impl, std::tuple<Ins...>, std::tuple<Outs...> >
{
    ...
    static void call_impl(GCUDAContext &ctx, detail::Seq<IIs...>, detail::Seq<OIs...>)
    {
    ...
    template<int... IIs, int... OIs>
    static void call_impl(cv::GCUDAContext &ctx, Impl& impl, detail::Seq<IIs...>, detail::Seq<OIs...>)
    {
    ...
    static void call(GCUDAContext &ctx)
    {
        call_impl(ctx,
    ...
    static void callFunctor(cv::GCUDAContext &ctx, Impl& impl)
    {
        call_impl(ctx, impl,
                  typename detail::MkSeq<sizeof...(Ins)>::type(),
                  typename detail::MkSeq<sizeof...(Outs)>::type());
...
class GCUDAKernelImpl: public cv::detail::CUDACallHelper<Impl, typename K::InArgs, typename K::OutArgs>,
                      public cv::detail::KernelTag
{
    using P = detail::CUDACallHelper<Impl, typename K::InArgs, typename K::OutArgs>;

public:
    using API = K;

    static cv::gapi::GBackend backend()  { return cv::gapi::cuda::backend(); }
    static cv::GCUDAKernel     kernel()   { return GCUDAKernel(&P::call);     }
};

#define GAPI_CUDA_KERNEL(Name, API) struct Name: public cv::GCUDAKernelImpl<Name, API>

class gapi::cuda::GCUDAFunctor : public gapi::GFunctor
{
public:
    using Impl = std::function<void(GCUDAContext &)>;

    GCUDAFunctor(const char* id, const Impl& impl)
        : gapi::GFunctor(id), impl_{GCUDAKernel(impl)}
    {
    }

    GKernelImpl    impl()    const override { return impl_;                }
    gapi::GBackend backend() const override { return gapi::cuda::backend(); }
...
#endif // OPENCV_GAPI_GCUDAKERNEL_HPP
